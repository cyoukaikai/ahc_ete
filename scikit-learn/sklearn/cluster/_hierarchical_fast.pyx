...
################################################################################
# Implementation by Kaikai Zhao (Tokyo University of Agriculture and Technology,
# cyoukaikai@gmail.com, 2020-10-29)
##############################################################################
# merge strategies implemented on IntFloatDicts for object tracking tasks.
...
@cython.boundscheck(False)
@cython.wraparound(False)
def max_merge_tracking(IntFloatDict a, IntFloatDict b,
              np.ndarray[ITYPE_t, ndim=1] mask,
              ITYPE_t n_a, ITYPE_t n_b):
    """Merge two IntFloatDicts with the max strategy: when the same key is
    present in the two dicts, the max of the two values is used.
    ...
    cdef cpp_map[ITYPE_t, DTYPE_t].iterator b_end = b.my_map.end()
    cdef ITYPE_t key
    # cdef DTYPE_t value  # not used
    # First copy a into out
    while a_it != a_end:
        key = deref(a_it).first
        if mask[key]:
            b_it = b.my_map.find(key)
            if b_it != b_end:
                # Key found
                out_obj.my_map[key] = fmax(deref(b_it).second, deref(a_it).second)
        inc(a_it)
    return out_obj
...
@cython.boundscheck(False)
@cython.wraparound(False)
def average_merge_tracking(IntFloatDict a, IntFloatDict b,
              np.ndarray[ITYPE_t, ndim=1] mask,
              ITYPE_t n_a, ITYPE_t n_b):
    """Merge two IntFloatDicts with the average strategy: when the same key is
    present in the two dicts, the average of the two values is used.
    ...
    while a_it != a_end:
        ...
        if mask[key]:
            ...
            if b_it != b_end:
                # Key found
                out_obj.my_map[key] = (n_b * deref(b_it).second + n_a *  deref(a_it).second) / n_out
                # print(n_a,  deref(a_it).second, n_b, deref(b_it).second, n_out, out_obj.my_map[key])
        inc(a_it)
    return out_obj
...
@cython.boundscheck(False)
@cython.wraparound(False)
def single_merge_tracking(IntFloatDict a, IntFloatDict b,
              np.ndarray[ITYPE_t, ndim=1] mask,
              ITYPE_t n_a, ITYPE_t n_b):
    """Merge two IntFloatDicts with the min strategy: when the same key is
    present in the two dicts, the min of the two values is used.
    ...
    while a_it != a_end:
        ...
        if mask[key]:
            ...
            if b_it != b_end:
                # Key found
                out_obj.my_map[key] = fmin(deref(b_it).second,  deref(a_it).second)
        inc(a_it)
    return out_obj
...
@cython.boundscheck(False)
@cython.wraparound(False)
def single_merge(IntFloatDict a, IntFloatDict b,
              np.ndarray[ITYPE_t, ndim=1] mask,
              ITYPE_t n_a, ITYPE_t n_b):
    """Merge two IntFloatDicts with the min strategy: when the same key is
    present in the two dicts, the min of the two values is used.
    ...
    while b_it != b_end:
        ...
        inc(b_it)
    return out_obj
############################################### Implementation by Kaikai Zhao (end)
